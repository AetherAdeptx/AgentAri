...
std::filesystem::path project_file(const std::filesystem::path& relative) {
    ...
    const std::filesystem::path source_path =
        std::filesystem::path(AGENTARI_PROJECT_SOURCE_DIR) / relative;
...
void print_state(const agentari::text::TokenizerNeuralState& state) {
...
void print_predictions(const std::vector<agentari::text::Prediction>& predictions) {
...
bool gpu_self_test(agentari::gpu::VulkanComputeBackend& gpu) {
    if (!gpu.available()) {
...
int main(int argc, char* argv[]) {
    ...
    using agentari::text::PredictorConfig;
    ...
    std::filesystem::path shader_path = executable_path.parent_path() / "agentari-matmul.comp.spv";
    agentari::gpu::VulkanComputeBackend gpu(shader_path.string());

    std::cout << "AgentAri toy\n"
              << "CPU SIMD: " << agentari::hardware::preferred_simd_backend() << "\n"
    ...
    while (std::cout << "> " && std::getline(std::cin, line)) {
        ...
        if (line.rfind("/learn ", 0U) == 0U) {
            ...
            predictor.apply_message_feedback(std::vector<float>(agentari::text::tokenizer_feature_count,
                                                                value));
